...
/* This range of timestamps are treated as synthetic messages (generated by
 * mlat, from UAT, etc)
 */
#define SYNTHETIC_TIMESTAMP_START 0xFF004D4C4154ULL
#define SYNTHETIC_TIMESTAMP_END (SYNTHETIC_TIMESTAMP_START + 10)
...
static PyMemberDef modesreaderMembers[] = {
    ...
    { "want_synthetic_messages", T_BOOL,    offsetof(modesreader, want_synthetic_messages), 0,       "should the decoder return synthetic messages?" },
...
static PyObject *modesreader_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    ...
    self->want_synthetic_messages = 0;
...
static int is_synthetic_or_zero_timestamp(unsigned long long timestamp)
{
    return (timestamp == 0 || is_synthetic_timestamp(timestamp));
}
...
{
    if (is_synthetic_or_zero_timestamp(timestamp))
        return 1;
...
#define ADVANCE \
        ...
        if (has_timestamp_signal && !is_synthetic_or_zero_timestamp(timestamp)) {
...
static int filter_message(modesreader *self, PyObject *o)
{
    ...
    if (is_synthetic_timestamp(message->timestamp) && !self->want_synthetic_messages) {
        return 0;
    }
